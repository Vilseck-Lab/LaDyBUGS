...
def calcFastMBAR(lambstates,beta,numCycle,numBiasLoops):
    ...
    if 0 in N_k: 
        ...
        # set "complete" values as the normal F and Fstdev values
        F = Fcomplete
        Fstdev = SDcomplete
    # END IF SOME STATES ARE SUNSAMPLED

    # IF ALL STATES ARE SAMPLED
    else:
        # construct a FastMBAR object with the energy matrix and the number of configuration array
        fastmbar = FastMBAR(energy = u_kn, num_conf = N_k, cuda=do_cuda, cuda_batch_mode=do_cuda_batch, bootstrap=do_bootstrap) 
        
        # calculate free energies by solving the MBAR equations
        F = fastmbar.F
        if do_bootstrap:
            Fstdev=fastmbar.F_std
    # END IF ALL STATES ARE SAMPLED
...
#    if N_k[0] == 0:
    ...
    fp=open("mbar_results/mbar."+str(numBiasLoops - 1)+".txt",'w')
