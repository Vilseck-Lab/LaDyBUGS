...
psf.setBox(box,box,box,90,90,90) # assumes cubic box

# quick check for multisite simulations
site2_check = len(glob(prep_root+'/site2*'))
if site2_check != 0:
    raise ValueError('multi-site has not been implemented yet')
    quit()
...
# List out Substituents by Name 
#name=[[]]                                         # name[0] = environment (emtpy list)
#name.append(['ATNM','ATNM',...])

# List out Substituents by Index-value from the CHARMM PSF
nameid=[[]]                                        # nameid[0] = environment (emtpy list)
nameid.append([2485,2486,2487,2488,2489])
nameid.append([2490,2491,2492,2493,2494,2495,2496,2497])
nameid.append([2498,2499,2500,2501,2502,2503,2504,2505,2506,2507,2508])
nameid.append([2509,2510,2511,2512,2513,2514,2515,2516,2517,2518,2519])
nameid.append([2520,2521,2522,2523,2524,2525,2526,2527,2528,2529,2530,2531,2532,2533])
nameid.append([2534,2535,2536,2537,2538,2539,2540,2541,2542,2543,2544,2545,2546,2547])


nsubs=len(nameid)-1 # assumes only single site perturbation
sub2site = [0]
for sub in range(nsubs):
    sub2site.append(1) # hardcoded for single site sim

if phiscale:
    # PHI SEARCH/TESTS
    
    # load in user-defined dihedral angles
    phis=[]
    phiIDK=[]
    for s in range(nsubs): # initialize variables
        phis.append([])
        phiIDK.append({})
    for line in open('dihedrals_to_scale.txt','r'): # load in data from file
        tmp=line.split()
    ...
    def phiSearch(aphi,phis):  # function to match up dihedrals
        """ Compare a phi from the system (aphi) to list of user specified phis (phis) """
        pchk=False
        for s in range(nsubs):
            for p in range(len(phis[s])):
                uphi=phis[s][p]
                rphi=[phis[s][p][i] for i in range(len(phis[s][p])-1,-1,-1)] # reverse atm order
                if (aphi == uphi): 
                    pchk=True
                elif (aphi == rphi): 
                    pchk=True
    ...
    # compare system dihedrals vs dihedrals to scale
    # match up system dihedral index values with substituent #
    for i in range(0,phif.getNumTorsions()):
        aphi = phif.getTorsionParameters(i)
        pchk,s = phiSearch(aphi[0:4],phis)
        if pchk:      # if we find a match, s=sub#(0-idx) and i=dihedral index
            phiIDK[s][i]=aphi[6].value_in_unit(aphi[6].unit)
    
    # write phiIDK to disk for reading by future LDB script
    pp=open('phiIDK.txt','w')
    for s in range(nsubs):
        for k in phiIDK[s].keys():
            pp.write("%d %d %.16f\n" % (s,k,phiIDK[s][k]))
    pp.close()


sele=[[] for i in range(0,len(nameid))]           #sele[0] = env indices
sele_all=[]
sele_env=[]
sele_lam=[]
...
for at in pdb.topology.atoms():
  found=0
  for i in range(1,len(nameid)):
    #if at.name in name[i] and int(at.residue.id)==resid1: # use for name
    if at.index in nameid[i]:  # use for nameid
