...
# read in protein - truncated system for pfkfb3
...
read.sequence_pdb(builddir+'/wata.pdb')
gen.new_segment('WATA', setup_ic=True, angle=False, dihedral=False)
read.pdb(builddir+'/wata.pdb',resid=True)
...
image.setup_segment(0.0, 0.0, 0.0, 'PROA')  # list out one by one
image.setup_segment(0.0, 0.0, 0.0, 'LIG')  # list out one by one
image.setup_residue(0.0, 0.0, 0.0, 'WATA')
